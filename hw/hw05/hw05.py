...
class VendingMachine:
    ...
    def add_funds(self, coins):
        if (self.stock == 0):
            return "Inventory empty. Restocking required. Here is your ${0}.".format(coins)
        self.funds += coins
        ...
        if (self.stock == 0):
            ...
            if (self.funds >= self.cost):
                ...
                if (self.funds > 0):
                    t = self.funds
                    self.funds = 0
                    return "Here is your {1} and ${0} change.".format(t, self.name)
                else:
...
def store_digits(n):
    ...
    def create(n, prev):
        if (n < 10):
            return Link(n, prev)
        return create(n // 10, Link(n % 10, prev))
    
    return create(n, Link.empty)
